...
** SUDO EDIT
Sometime we have to edit something with sudo but eamcs do not have any sudo support, So we need to add this so that we can edit files which need sudo privilage.
#+begin_src emacs-lisp
