...
* SETUP
I am using ~arch liux~ as my main os. Top of that I am using ~EMACS~ everyhing. For setting up emacs first you have to download emacs in your system in arch install it with pacman.
#+begin_src shell
sudo pacman -S emacs
#+end_src

Then you have to make a directory and some files for configureing your emacs. Before making any changes first conform that you don't have any previos emacs config file like the ~.emacs.d~ or ~.config/emacs~. To be sure just run this command's:
#+begin_src shell
sudo rm -rf ~/.emacs.d/ ~/.config/emacs/
#+end_src

The make a directory for emacs and make 3 files ~init.el~, ~early-init.el~ and ~config.org~ because we are going to configure our emacs with an orgmode file.

#+begin_src shell
sudo mkdir -p ~/.config/emacs/
cd ~/.config/emacs/
touch init.el early-init.el config.org
#+end_src

* CONFIGUREING EMACS
** INIT.EL FILE
At first you have to configure your init.el file. just add those line it will tell emacs that we will add our all configuretion in ~config.org~ file.
...
This will tell emacs that all the configuretion should load from config.org file.

** EARLY-INIT.EL
We need another file because we are going to use ~ELPACA~ for our package system. So we have to add those line in the ~early-init.el~ file.
#+begin_src emacs-lisp
...
** ALL THE REPOS
So this is ~elpaca~ which is the main repo of our emacs config . If there are any issue visit [[https://github.com/progfolio/elpaca][elpaca]]. 
*** ELPACA
#+begin_src emacs-lisp
...
*** SOME REPO
This are for some extra elisp repo for require package.
#+begin_src emacs-lisp
(require 'package)
...
Doom Modeline and Doom Themes are for a good vibe of emacs theme and the modeline.
#+begin_src emacs-lisp
;;======= DOOM MODE LINE =======
...
** FONTS 
If you are having trouble of loading the themes please uncomment this line in your early-init.el file
#+begin_example
(add-to-list 'default-frame-alist '(font . "JetBrains Mono-11"))
#+end_example
...
** ORG MODE
Here are some package and configuretion for working with org mode files.
*** ORG MAIN
#+begin_src emacs-lisp
(use-package toc-org
  :ensure t
...
Here are the sttings for minimal emacs.
...
** MINIBUFFER ESCAPE
This setting is for escapeing from the minibuffer with the ~ESC~ button.
#+begin_src emacs-lisp
(global-set-key [escape] 'keyboard-escape-quit);;MInibuffer escape
#+end_src

** EVIL MODE 
Evil mode give the ability for using vim keybinding in emacs.
#+begin_src emacs-lisp
(use-package evil
  :ensure t
  :init
...
Ivy & Counsel in Emacs

~Ivy~ is a powerful and lightweight completion framework for Emacs, making the minibuffer smarter and faster with fuzzy matching and dynamic suggestions.

~Counsel~ extends Ivy by providing enhanced versions of common Emacs commands like `M-x`, `find-file`, and `grep`, offering a more intuitive and productive workflow.

Together, they improve navigation, searching, and overall user experience in Emacs.
...
Here are the keybinding's which I took from ~distro tube~ eamcs config. I use package general for making this keybinding.
#+begin_src emacs-lisp
(use-package general
  :ensure t
  :config
...
** SUDO EDIT
Sometime we have to edit something with sudo but eamcs do not have any sudo support by default, So we need to add this so that we can edit files which need sudo privilage.
#+begin_src emacs-lisp
...
Those are my personal keybindings with Meta/Alt as a leader key.
#+begin_src emacs-lisp
...
** BLANK BUFFER
This is my personal setup. It is a blank buffer which have 100% transparency, so we can see our wallpaper through it.
#+begin_src emacs-lisp
...
** TRANSPERENCY
Transparency and effect with picom.
#+begin_src emacs-lisp
